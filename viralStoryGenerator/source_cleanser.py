...
def cleanse_sources(raw_sources, endpoint, model, temperature=0.7):
    """
    Summarizes or 'cleans up' the given raw_sources using your local LLM.
...
def chunkify_and_summarize(raw_sources, endpoint, model,
    ...
    if len(chunks) == 1:
        return cleanse_sources(chunks[0], endpoint, model, temperature)
    ...
    for i, chunk in enumerate(chunks, start=1):
        ...
        chunk_summary = cleanse_sources(chunk, endpoint, model, temperature)
    ...
    final_summary = cleanse_sources(all_partial_text, endpoint, model, temperature)
